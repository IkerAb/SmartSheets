...
// Vista de calendario fiscal con ventas por semana y promos
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        setWeeks(data.weeks ?? []);
        // seleccionar la semana con más ventas por default
    ...
    return (
      <div className="flex flex-col items-center justify-center py-32 text-center">
        ...
        <p className="text-slate-500 text-sm max-w-sm">
          Necesitas subir un archivo de ventas y un calendario de promociones para ver esta vista.
        </p>
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-4">
          ...
          <p className="text-white text-2xl font-bold">{weeksWithPromo} <span className="text-slate-500 text-sm">/ {weeks.length}</span></p>
        </div>
        <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-4">
        ...
        <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-4">
          ...
          <p className="text-emerald-400 text-xs">${bestWeek?.total_sales.toLocaleString("en-US", { maximumFractionDigits: 0 })}</p>
        </div>
      </div>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-slate-500 text-xs">{w.fecha_inicio}</p>
                    </div>
                  </div>
